        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Total: <span className={cn("font-semibold", totalPercentage === 100 ? "text-emerald-400" : "text-amber-400")}>{totalPercentage}%</span>
                {totalPercentage !== 100 && <span className="text-amber-400 ml-1">(deve somar 100%)</span>}
              </CardDescription>
            </div>
            ...
            <div key={p.id} className="flex items-center gap-3 p-3 rounded-xl border border-[var(--border)] bg-[#0d0d14]">
              ...
              <div className="flex-1 flex items-center gap-2">
                ...
                <input
                  ...
                  className="w-16 h-8 rounded-lg border border-[var(--border)] bg-[#111118] text-center text-sm text-white outline-none focus:border-[#6366f1]"
                  min={0} max={100}
                />
                <span className="text-xs text-[#71717a] w-4">%</span>
              </div>
              {platforms.length > 1 && (
                <button onClick={() => removePlatform(p.id)} className="text-[#52525b] hover:text-red-400 transition-colors cursor-pointer">
          ...
          <Card key={p.id} className="border-l-4" style={{ borderLeftColor: p.color }}>
            <CardContent className="p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Orçamento Total", value: `R$ ${p.share.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}` },
                  { label: "Diário (período)", value: `R$ ${p.dailyBudget.toFixed(2)}` },
                  { label: "Projeção Mensal", value: `R$ ${p.monthlyProjection.toFixed(2)}`, highlight: true },
                  { label: "Restante do Mês", value: `R$ ${p.remainingBudget.toFixed(2)}` },
                ].map((item) => (
                  <div key={item.label} className={cn("rounded-lg p-3", item.highlight ? "bg-[#6366f1]/10 border border-[#6366f1]/20" : "bg-[#0d0d14] border border-[var(--border)]")}>
                    <p className="text-[10px] text-[#71717a] mb-1">{item.label}</p>
                    <p className={cn("text-sm font-bold", item.highlight ? "text-[#818cf8]" : "text-white")}>{item.value}</p>
                  </div>
                ))}
              </div>
            </CardContent>
        ...
        <Card className="bg-[#6366f1]/5 border-[#6366f1]/20">
          <CardContent className="p-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: "Total no Período", value: `R$ ${budget.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}` },
                { label: "Média Diária", value: `R$ ${(budget / daysInPeriod).toFixed(2)}` },
                { label: "Projeção Mensal", value: `R$ ${((budget / daysInPeriod) * daysInMonth).toFixed(2)}` },
                { label: "Restante do Mês", value: `R$ ${((budget / daysInPeriod) * daysRemaining).toFixed(2)}` },
              ].map((item) => (
                <div key={item.label}>
